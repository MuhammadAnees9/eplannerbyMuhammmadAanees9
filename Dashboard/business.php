...
<style type="text/css">
    ...
    $uid = $_SESSION['user']['id'];
    $q = mysqli_query($con, "SELECT `business`.*, `services`.sname FROM `business` LEFT JOIN `services` ON `services`.bid=`business`.id WHERE `business`.ownerId='$uid' GROUP BY `business`.id");
    $i=0;
    while($res = mysqli_fetch_assoc($q)){
        $i++;
    ?>
<h2><?php echo $i.'-'.$res['bname'] ?></h2>
<h4>Services</h4>
<ul>
