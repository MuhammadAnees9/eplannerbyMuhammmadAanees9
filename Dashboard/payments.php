...
<style type="text/css">
    ...
    // gathering data about recipt
        ...
        $q = mysqli_query($con, "SELECT `bookings`.*, `business`.* FROM `bookings` LEFT JOIN `business` ON `bookings`.bid=`business`.id WHERE `bookings`.id='$bn'");
...
<div class="form-group col-sm-6">
  ...
  <div id="menu2" class="tab-pane fade">
    ...
    if(isset($_POST['easypaisa'])){
      ...
      $amount = $_POST['amount'];
      $easy = mysqli_query($con, "INSERT INTO `payments`(cardNumber, amount, method, uid, bid) VALUES('$account', '$amount', 'EasyPaisa', '$uid', '$bn')");
      if($easy)
      {
        $up = mysqli_query($con, "UPDATE `bookings` SET bstatus='Approved' WHERE uid='$uid' AND id='$bn'");
        if($up)
